...
# functions creations
def condit_annont(file_name):
    # Insertions for conditions table
    with open(file_name, 'r') as r1:
        process = 0
        total = len(open(file_name).readlines())
...
def yeast_gene_combine(file_name):
    ...
    with open(file_name, 'r') as r1:
        ...
        for line in r1:
            ...
            cur.execute(
                ...
                (ID_num, bp, cc, mf))
            cur.execute('INSERT IGNORE INTO YeastGene_Localization(Gene_ID,Localization_ID) VALUES (%s,%s)',
                        (gene, ID_num))
            process += 1
            print('Process: ' + str(process) + ' ' + 'Total: ' + str(total))


def sc_expression(file_name):
